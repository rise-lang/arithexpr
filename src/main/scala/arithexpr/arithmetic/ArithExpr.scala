...
abstract sealed class ArithExpr {
  ...
  def substitute(subs: scala.collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr]
...
case object ? extends ArithExpr with SimplifiedExpr {
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = None
}
...
case class IntDiv private[arithmetic](numer: ArithExpr with SimplifiedExpr, denom: ArithExpr with SimplifiedExpr)
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val n = numer.substitute(subs)
    val d = denom.substitute(subs)
    if (n.isEmpty && d.isEmpty) {
      None
    } else {
      Some(n.getOrElse(numer) / d.getOrElse(denom))
    }
  }
}
...
case class Pow private[arithmetic](b: ArithExpr with SimplifiedExpr, e: ArithExpr with SimplifiedExpr)
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val bs = b.substitute(subs)
    val es = e.substitute(subs)
    if (bs.isEmpty && es.isEmpty) {
      None
    } else {
      Some(bs.getOrElse(b).pow(es.getOrElse(e)))
    }
  }
}
...
case class Log private[arithmetic](b: ArithExpr with SimplifiedExpr, x: ArithExpr with SimplifiedExpr)
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val bs = b.substitute(subs)
    val xs = x.substitute(subs)
    if (bs.isEmpty && xs.isEmpty) {
      None
    } else {
      Some(Log(bs.getOrElse(b), xs.getOrElse(x)))
    }
  }
}
...
case class Prod private[arithmetic](factors: List[ArithExpr with SimplifiedExpr]) extends ArithExpr {
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val (same, fs) = factors
      .foldLeft((true, Seq[ArithExpr]())) { case ((same, fs), x) =>
        val xs = x.substitute(subs)
        (same && xs.isEmpty, fs :+ xs.getOrElse(x))
      }
    if (same) {
      None
    } else {
      val r = Some(fs.reduce(_ * _))
      r
    }
  }
...
case class Sum private[arithmetic](terms: List[ArithExpr with SimplifiedExpr]) extends ArithExpr {
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val (same, ts) = terms
      .foldLeft((true, Seq[ArithExpr]())) { case ((same, ts), x) =>
        val xs = x.substitute(subs)
        (same && xs.isEmpty, ts :+ xs.getOrElse(x))
      }
    if (same) {
      None
    } else {
      val r = Some(ts.reduce(_ + _))
      r
    }
  }
}
...
case class Mod private[arithmetic](dividend: ArithExpr with SimplifiedExpr, divisor: ArithExpr with SimplifiedExpr)
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val dds = dividend.substitute(subs)
    val dss = divisor.substitute(subs)
    if (dds.isEmpty && dss.isEmpty) {
      None
    } else {
      Some(dds.getOrElse(dividend) % dss.getOrElse(divisor))
    }
  }
}
...
case class FloorFunction private[arithmetic](ae: ArithExpr with SimplifiedExpr) extends ArithExpr {
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] =
    ae.substitute(subs).map(floor(_))
}
...
case class IfThenElse(test: BoolExpr, t: ArithExpr with SimplifiedExpr, e: ArithExpr with SimplifiedExpr) extends ArithExpr with SimplifiedExpr {
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val is = test.substitute(subs)
    val ts = t.substitute(subs)
    val es = e.substitute(subs)
    if (is.isEmpty && ts.isEmpty && es.isEmpty) {
      None
    } else {
...
case class BitwiseXOR private[arithmetic](a: ArithExpr with SimplifiedExpr, b: ArithExpr with SimplifiedExpr)
  ...
  override def substitute(subs: collection.Map[ArithExpr, ArithExpr]): Option[ArithExpr] = {
    val as = a.substitute(subs)
    val bs = b.substitute(subs)
    if (as.isEmpty && bs.isEmpty) {
      None
    } else {
      Some(as.getOrElse(a) ^ bs.getOrElse(b))
    }
  }
}
...
class NamedVar(override val name: String, override val range: Range = RangeUnknown, val default: Option[Int] = None, override val fixedId: Option[Long] = None) extends Var(name, range, fixedId) {
  ...
  override def copy(r: Range) = new NamedVar(name, r, default)

  override def cloneSimplified() = new NamedVar(name, range, default) with SimplifiedExpr
}
...
object NamedVar {
  ...
  def apply(name: String, range: Range, default: Option[Int]): NamedVar = new NamedVar(name, range, default)
}
...
/**
  ...
  */
case class InclusiveIndexVar(override val name: String, from: ArithExpr, upTo: ArithExpr, override val default: Option[Int] = None, override val fixedId: Option[Long] = None) extends NamedVar(name, RangeAdd(from, upTo + 1, 1), default, fixedId) {
...
object BigSum {
  def apply(from: ArithExpr, upTo: ArithExpr, makeBody: InclusiveIndexVar => ArithExpr): SimplifiedExpr = {
    val freshID = Var.incCnt()
    val idxVar = InclusiveIndexVar(s"sumIdx$freshID", from, upTo) //Make sure start does not overlap stop?
