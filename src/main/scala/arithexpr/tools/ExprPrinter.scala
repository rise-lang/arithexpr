...
import arithexpr.arithmetic.BoolExpr.True
...
object ExprPrinter {
  ...
  def dump(boolExpr: BoolExpr, depth:Int):Unit = {
    boolExpr match {
      case BoolExpr.True => println(" |" * depth + "- true")
      case BoolExpr.False => println(" |" * depth + "- false")
